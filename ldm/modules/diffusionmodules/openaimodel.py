...
class SegBlock(nn.Module):
    def __init__(self, in_channels, out_channels, con_channels,
        ...
        # self.bn1 = layers.ccbn(in_channels, con_channels, self.which_linear, eps=1e-4, norm_style='bn')
        # self.bn2 = layers.ccbn(out_channels, con_channels, self.which_linear, eps=1e-4, norm_style='bn')
        # upsample layers
        self.upsample = upsample
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None,class_token_index=[]):
        #attn_list=[]
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context,class_token_index=class_token_index)
...
class UNetModel(nn.Module):
    ...
    #@profile
    def forward(self, x, timesteps=None, context=None, y=None,class_token_index=[],**kwargs):
        """
